...
2. Automatic format preservation (styles, images, merged cells)
...
def translate_text_with_gemini(texts, source_lang="auto", target_lang="en", model_name="gemini-1.5-flash"):
    ...
    prompt = f"""
        ...
        ONLY RETURN AN EMPTY STRING IF THE REQUEST TEXT IS EMPTY
...
def process_sheet(orig_sheet, trans_sheet, final_sheet):
    """Process individual sheet - now mostly for dimensions (not used in post-processing logic anymore)"""
    try:
        # Merged cells and value copy moved to post_process_translation
        # Only keeping dimension copy here if needed in the future, but post_process handles value and merged cells.

        # Copy dimensions -  (Dimensions are now copied in post_process, keeping this for potential future use if needed separately)
...
def translate_excel(input_file, source_lang, target_lang, model_name, batch_size, post_process):
    ...
    try:
        ...
        for sheet_name in tqdm(wb.sheetnames, desc="Processing Content"):
            process_worksheet(wb[sheet_name], wb_out.create_sheet(sheet_names[sheet_name]),
                            source_lang, target_lang, model_name, batch_size)
...
def process_worksheet(source_sheet, target_sheet, src_lang, tgt_lang, model_name, batch_size):
    """Process individual worksheet using Gemini API"""
    # Copy styles
    for row in source_sheet.iter_rows():
        for cell in row:
...
def main():
    ...
    translate_parser.add_argument('--model', default='gemini-2.0-flash',
